const Network = require('./learning.js');
const setGen = require('./setGenerator.js');

const debugArg = process.argv.indexOf('-d');
if(debugArg > -1)
	Network.logLevel = process.argv[debugArg + 1];
...
const loss = (e, o) =>
	e * sigmoid(o) * (1- sigmoid(o))
const lossP = (childErr, childSum, parentOutput) =>
	childErr * sigmoidPrime(childSum) * parentOutput 

const makeNetworks = (n, layers) => {
	return Array.from(Array(n).keys(), () =>
		new Network(layers, sigmoid, sigmoidPrime, loss, lossP));
};
...
const learnMutualXOR = () => {
	const layers = [3, 15, 2];
	learn(layers, 0.2, 0.2, 210000, 100, 9, setGen.mutualXOR);
};
